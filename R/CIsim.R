"CIsim" <-
function(samples=100, n=30, parameter=0.5, sigma=1, conf.level = 0.95, type ="Mean")
{
Adkblue <- "#0080FF"
Aorange <- "#FF4C0C"
alpha <-1-conf.level
...
title(main=bquote(paste(.(N), " random ", .(CL), "% confidence intervals where ", mu ,  " = ", .(parameter) )))
        for(i in 1:N)
          {
            low<-ll[i];
            high<-ul[i];
...
title(main=bquote(paste(.(N), " random ", .(CL), "% confidence intervals where ", pi,  "=", .(parameter) )))
        for(i in 1:N)
        {
        low<-ll[i]
        high<-ul[i]
